<?php
/*
Plugin Name: Sync Post With Other Site Extended by Logico
Description: Please First Install and activate "Sync Post With Other Site" plugin, This is Child plugin of "Sync Post With Other Site" 
*/
...
function my_plugin_admin_notices() {
    if (!is_plugin_active('sync-post-with-other-site/SyncPostWithOtherSite.php')) {
        echo "<div class='error'><p>Please Activate Sync Post With Other Site Plugin</p></div>";
    }
}
    ...
    class SPS_Sync {
        ...
        function sps_send_data_to( $action, $args = array(), $sps_website = array() ) {
            ...
            if( !empty( $general_option ) && isset( $general_option['sps_host_name'] ) && !empty( $general_option['sps_host_name'] ) ) {
                ...
                if( in_array($sps_value, $sps_website) ) {

                     $args['sps']['host_name'][]   = !empty( $sps_value ) ? $sps_value : array();
        ...
        function sps_remote_post( $action, $args = array() ) {
            ...
            do_action( 'spsp_before_send_data', $args );
             ...
             $sps_setting = get_option('sps_setting');
             foreach ($args['sps']['host_name'] as $key => $value) {


                if (false !== $key = array_search($value, $sps_setting['sps_host_name'])) {

                       // update_post_meta(5,'host_url_'.$key ,$value);

                    // update_post_meta(5,'sps_content_username_'.$key ,$sps_setting['sps_content_username'][$key]);

                    // update_post_meta(5,'sps_content_password_'.$key ,$sps_setting['sps_content_password'][$key]);


                    $args['sps']['content_username'] = $sps_setting['sps_content_username'][$key];

                    $args['sps']['content_password'] = $sps_setting['sps_content_password'][$key];



                    // update_post_meta(5,'qaz2'.$key ,$args);

                    $url = $value."/?sps_action=".$action;

                     $responce = wp_remote_post( $url, array( 'body' => $args ));
                ...
                // }
                 ...
                 // if(in_array($value, $sps_setting['sps_host_name'])){
